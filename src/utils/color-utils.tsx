export const getClosestCSSColorName = (hexColor) => {
  // Define an object that maps CSS color names to their hex values
  const cssColors = {
    aliceblue: "#f0f8ff",
    antiquewhite: "#faebd7",
    aqua: "#00ffff",
  ...
  // Iterate over CSS colors and find closest match
  for (const colorName in cssColors) {
    const [cr, cg, cb] = hexToRgb(cssColors[colorName]);
...
const getRandomRadialGradientDirection = (pctg = false): string => {
  ...
  const randomDirection = pctg
    ? generateRandomDirection(1, 100)
    : directions[Math.floor(Math.random() * directions.length)];
  return `${randomShape} at ${randomDirection}`;
};
